...
def apply_glitch_stripes(src, dst, h, w, orientation, strand_val, rand_lines, val):
    ...
    elif orientation == "Bande Temporali":
        # Temporal Band Slicer: bande orizzontali di altezza variabile (rand_lines
        # decide se irregolari), ciascuna banda pesca da un'altra posizione dello
        # STESSO frame — spostamento orizzontale O verticale, mai insieme sulla
        # stessa banda (rif. Snorpey / Rosa Menkman). Alcune bande restano pulite:
        # la probabilita' di spostamento scala con l'intensita' (val).
        prob_band = float(np.clip(0.15 + 0.8 * val, 0.1, 0.95))
        max_shift_x = max(10, int(w * (0.05 + 0.55 * min(val, 1.0))))
        max_shift_y = max(6, int(h * (0.03 + 0.45 * min(val, 1.0))))
        frame = src.copy()
        for s, e in get_b(h):
            if random.random() < prob_band:
                ...
                frame[s:e, :] = np.roll(src, (dy, dx), axis=(0, 1))[s:e, :]
    else:
        frame = src.copy()
                ...
                if slide_trans_type == "Glitch Burst":
                    ...
                    if stripe_mode and stripes:
                        if stripe_glitch:
                            ...
                            stripe_glitch_sources = {'Calderone': {geo: apply_glitch_stripes(base, base, h, w, geo, strand_val, rand_lines, intensity)
                                                      for geo in _geo_needed}} if _geo_needed else {}
                        else:
                    ...
                    if stripe_mode and stripes:
                        if stripe_glitch:
                            _geo_needed = {s.get('geometria', 'Uguale al Calderone') for s in stripes
                                           if s.get('source', 'Calderone') == 'Calderone'} - {'Uguale al Calderone'}
                            stripe_glitch_sources = {'Calderone': {geo: apply_glitch_stripes(blend, blend, h, w, geo, strand_val, rand_lines, intensity)
                                                      for geo in _geo_needed}} if _geo_needed else {}
                        else:
                            stripe_glitch_sources = {}
    ...
    # =========================================================
        ...
        def make_frame(t):
            ...
            if orientation == "Orizzontale":
                ...
                # Temporal Band Slicer: bande orizzontali di altezza variabile,
                # ciascuna ricollocata da un'altra posizione dello STESSO frame —
                # spostamento orizzontale O verticale, mai insieme sulla stessa
                # banda (rif. Snorpey / Rosa Menkman). Random module -> le bande
                # cambiano ad ogni frame invece di restare congelate nel video.
                target = pick()
                prob_band = float(np.clip(0.15 + 0.8 * val, 0.1, 0.95))
                max_shift_x = max(10, int(w * (0.05 + 0.55 * min(val, 1.0))))
                max_shift_y = max(6, int(h * (0.03 + 0.45 * min(val, 1.0))))
                for s, e in get_b(h):
                    if random.random() < prob_band:
                        if random.random() < 0.5:
                            ...
                            dx, dy = random.randint(-max_shift_x, max_shift_x), 0
                        frame[s:e, :] = np.roll(target, (dy, dx), axis=(0, 1))[s:e, :]
                    else:
                        frame[s:e, :] = target[s:e, :]
                ...
                if stripe_glitch:
                    ...
                    for _src_name, _src_frame in (('Calderone', calder_clean),) + (('Calderone 2', _extra_src.get('Calderone 2')),):
                        ...
                        _geo_needed = {s.get('geometria', 'Uguale al Calderone') for s in stripes
                                       if s.get('source', 'Calderone') == _src_name} - {'Uguale al Calderone'}
                        if _geo_needed:
                            stripe_glitch_sources[_src_name] = {geo: apply_glitch_stripes(_src_frame, _src_frame, h, w, geo, strand_val, rand_lines, val)
                                                                 for geo in _geo_needed}
                else:
                    stripe_glitch_sources = {}
    ...
    if calderone2_on:
        ...
        if calderone2_cfg['split_on']:
            calderone2_cfg['split_orient'] = st.radio("Direzione split", 
                ["Verticale (sx/dx)", "Orizzontale (alto/basso)", "Scacchiera (griglia)"],
                horizontal=True, key="calderone2_split_orient")
            if calderone2_cfg['split_orient'] == "Scacchiera (griglia)":
                _gc1, _gc2 = st.columns(2)
                with _gc1:
                    calderone2_cfg['split_grid_rows'] = st.slider("Righe", 1, 10, 2, key="calderone2_grid_rows")
                with _gc2:
        ...
        if stripe_mode:
            ...
            for _loop_idx, i in enumerate(list(st.session_state.stripe_ids)):
                ...
                with st.expander(_exp_title, expanded=(_loop_idx == 0), key=f"exp_stripe_{i}"):
                    ...
                    if s_dict['source'] in ("Calderone", "Calderone 2"):
                        s_dict['geometria'] = st.selectbox("🎛️ Geometria di questa striscia",
                            ...
                            help="Solo se '⚡ Striscia glitchata' è ON in alto: quale algoritmo di glitch "
                                 "usare DENTRO questa striscia (si applica alla sorgente scelta sopra, "
                                 "sia Calderone che Calderone 2), indipendente dalle altre strisce e dallo sfondo. "
                                 "'Uguale al Calderone' = nessun glitch extra, contenuto pulito.")
...
with c2:
    ...
    mode   = st.radio("Geometria", ["Orizzontale", "Verticale", "Mix (H+V)", "Mosaico", "Bande Temporali", "Nessun Effetto"],
        help="Bande Temporali: bande orizzontali irregolari che pescano da un'altra zona della stessa foto "
             "(orizzontale O verticale, mai insieme) — effetto tipo tracking error VHS, diverso dagli altri.")
