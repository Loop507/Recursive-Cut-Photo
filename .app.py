...
def generate_master(up_m1, up_m2, up_trit, up_aud, orientation, strand_val, max_limit, k_p, m1_s, m1_e, m2_s, m2_e, p_m1_ctrl, p_m2_ctrl, start_c, end_c, format_type, inc_master, rand_lines, chaos_val):
    fps = 24
    ...
    def make_frame(t):
        ...
        # --- LOGICA DI TRANSIZIONE STATISTICA (Tua Richiesta) ---
        if prog < start_c:
            prob_m1, prob_m2, prob_c = 1.0, 0.0, 0.0
        elif prog > end_c:
            prob_m1, prob_m2, prob_c = 0.0, 1.0, 0.0
        else:
            # Calcolo densità dei frame basato sugli slider percentuale
        ...
        # Magnetismo (Force Shift)
        mag1 = (m1_s + prog * (m1_e - m1_s)) / 100
        mag2 = (m2_s + prog * (m2_e - m2_s)) / 100
        dist_mult = 1.0 - np.clip(mag1 + mag2, 0, 0.95)
        
        # Smorzamento alle estremità per immagine sana
        if prog < (start_c * 0.5) or prog > (end_c + (1-end_c)*0.5): val *= 0.05

        def pick():
            r = random.random()
            if img_m1 is not None and r < prob_m1: return img_m1
            if img_m2 is not None and r < (prob_m1 + prob_m2): return img_m2
...
:: PROCESSO: Decadenza e Nascita Statistica
:: CAOS RANGE: {int(start_c*100)}% -> {int(end_c*100)}%
:: FRAME DENSITY: M1 {p_m1_ctrl}% | M2 {p_m2_ctrl}%
...
with c3:
    ...
    if st.button("🚀 AVVIA DISSEZIONE"):
        v, r = generate_master(up_m1, up_m2, up_t, up_a, mode, lines, dur, k_params, m1_s, m1_e, m2_s, m2_e, p_m1, p_m2, start_t, end_t, fmt, inc_m, rand_l, chaos)
